...
- optimization有问题
  ...
  无法下降不一定是遇到了critical point，如下图。此时需要调整学习率  
  ![alt text](images/image-14.png)
  - **Momentum**：相当于给原来的梯度下降增加一个**惯性**  
  ![alt text](images/image-13.png)  
  - 自适应学习率：在平缓方向的学习率尽量大，在陡峭方向的学习率尽量小  
...
- 如何判断是那种问题  
    - 比较不同层数的模型  
    如果更深的模型训练效果反而差，说明可能是优化问题，因为深层模型有更好的弹性  
    ![alt text](images/image-1.png)
## 训练集上损失小，测试集上损失大
- **过拟合（overfitting）**
![alt text](images/image-2.png) 
损失大小与模型复杂度的关系：![alt text](images/image-30.png)  
解决方法：  
  - 数据方面：增加训练数据 
...
## 训练集和验证集划分
  将训练集分为Training Set  和 Validation Set
 - N折交叉验证  
 ![alt text](images/image-6.png)  
 ## Batch的作用
   相关概念：shuffle：每一个epoch重新分配batch
   ...
   ## SoftMax
   - 二分类问题：直接使用**sigmoid函数**，将任意实数映射成为一个（0,1）区间的数值，一个0,1）区间的数值恰好可以用来表示概率：![alt text](images/image-21.png)
   ...
   效果：让梯度下降收敛更快  
   **在深度学习中需要对每一层都做标准化处理；**  
   标准化使得一个**batch**中的**所有输入彼此关联**，形成一个整体网络。  
   因为标准化后的输入均值为0，为了防止这一特征对网络造成负面影响，需要引入γ、β进行调整：  
   ![alt text](images/image-27.png)  
   - 在测试时通常没有batch，μ和σ怎么获得呢？指数平均  
