...
3. **最优化（Optimization）**  
    ...
    - 梯度下降法步骤：  
      ...
      更新w（η为超参数**学习率**，影响每次更新的步长，是一种**超参数**）：  
      ![alt text](image/image-1.png)
      - c. 反复更新w  
      停止更新：达到最大迭代次数或刚好某处微分为0  
    ![alt text](image/image-2.png)
## 模型（Model）  
线性模型（Linear Model）：参数量太少，有时会导致**模型偏差**（model bias）过大，无法模拟真实情况  
![alt text](image/image-3.png)  
可以使用组合方式表示其他类型函数：  
![alt text](image/image-4.png)  
以下**Piecewise Linear**函数（分段线性函数）都可以如此组合：
![alt text](image/image-5.png)
可以用Piecewise Linear逼近为Piecewise Linear曲线，进一步逼近任何连续曲线：  
![alt text](image/image-6.png)  
**sigmoid 函数**：![alt text](image/image-7.png)  
    z趋于负无穷，函数值趋于0；z趋于正无穷，函数值为1; 
...
**batch**: 将全部训练数据分成若干部分，每部分为一个batch    
**epoch**：把所有batch遍历一遍  
**update**：每一次更新参数  0
步骤：  
- 选取初始值θ<sub>0</sub>
